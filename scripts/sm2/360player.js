...
function ThreeSixtyPlayer() {
  ...
  this.config = {
    ...
    loadRingColor: '#FFCC99', // how much has loaded #ccc
    playRingColor: '#FF9900', // how much has played #000
    backgroundRingColor: '#FFFF99', // color shown underneath load + play ("not yet loaded" color)#eee
    ...
    peakDataLineRatio: 0.7,

    useAmplifier: true, // "pulse" like a speaker

    fontSizeMax: null, // set according to CSS
	
	iniVolume : 90, // by Jnom: this for simple volume handle
  ...
  this.events = {
    ...
    finish: function() {
      ...
      if (pl.config.playNext) {
		...
		//--- by Jnom: check if manual navigation to other aya
		if(JQ.getAyaFName(gAyaPs,gSuraID) != pl.lastSound.url){	
			pl.handleClick({target:je('soundlink'),button:1}); 
		}else{
			JQ.gotoNextAya(function(){
				pl.handleClick({target:je('soundlink'),button:1}); 
			});
  ...
  this.handleClick = function(e,tarjet) {
	//---- by Jnom
	if(!tarjet){tarjet=0};
  ...
  this.drawSolidArc = function(oCanvas, color, radius, width, radians, startAngle, noClear) {
    ...
    if (color) {
      ctx.fillStyle = color;
        ...
        /*uGrad = oCanvas.createLinearGradient(0, 16, 0, 0);

        uGrad.addColorStop(0,'rgb(0,192,0)');
        uGrad.addColorStop(0.30,'rgb(0,255,0)');
        uGrad.addColorStop(0.625,'rgb(255,255,0)');
        uGrad.addColorStop(0.85,'rgb(255,0,0)');
