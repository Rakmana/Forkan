...
class dbase {
	...
	*/
	...
	if(!dbase::isConnected()){//if not connected then connect
		$host = konfig::get('dbhost');
		$user = konfig::get('dbuser');
		$pass = konfig::get('dbpass');
		$base = konfig::get('dbbase');
		$GLOBALS['DBlinkID'] = @mysqli_connect("$host","$user","$pass") or die(debugger::halt("JBase::ConnectDB : <br />" .(konfig::isOn('error_mysql')? (mysqli_connect_error()) : 'JME-'.mysqli_connect_errno())));
	...
	*/
	public static function jError($text){
		 debugger::halt('<div style="padding-left:40px;text-align:left;">JBase::Error: <br />'.(konfig::isOn('error_mysql')? $text."<br />JError:  <small>".mysqli_error($GLOBALS['DBlinkID'])."</small>" : 'JME-'.mysqli_errno($GLOBALS['DBlinkID']))."</div>");
	}
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	/**
